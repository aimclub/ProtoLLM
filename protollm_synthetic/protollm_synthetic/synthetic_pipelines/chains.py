...
import logging
from protollm_synthetic.synthetic_pipelines.prompts import (
    generate_summary_system_prompt,
    generate_summary_evaluation_system_prompt,
    generate_rag_system_prompt,
    check_summary_quality_human_prompt,
    generate_rag_human_prompt,
    generate_aspect_summarisation_prompt,
    generate_summary_human_prompt,
    generate_aspect_summarisation_evaluation_system_prompt,
    generate_quiz_system_prompt,
    generate_quiz_human_prompt,
    generate_instruction_one_shot_system_prompt,
    generate_instruction_one_shot_human_prompt,
    merge_instructions,
    merge_instructions_human_prompt,
)
...
import random
from protollm_synthetic.synthetic_pipelines.schemes import (
    SummaryQualitySchema,
    RAGScheme,
    AspectSummarisationQualitySchema,
    QuizScheme,
    FreeQueryScheme,
    FreeQueryMerger,
)
...
class BaseChain:
    ...
    def _prepare_prompt(
        self, system_template, human_template, x: Dict[str, Any]
    ) -> PromptValue:
        prompt = ChatPromptTemplate(
            messages=[
                SystemMessagePromptTemplate.from_template(
                    [{"text": system_template.replace('"', "")}]
                ),
    ...
    async def _run_chain_with_retries(
        self,
        chain: Runnable,
        ...
        while attempt < retry_attempts:
            try:
                ...
                logger.warning(
                    f"Unexpected error during chain execution: {str(e)}", exc_info=False
                )
    ...
    def create_chain(
        ...
        prompt = RunnableLambda(
            lambda x: self._prepare_prompt(system_template, human_template, x)
        )

        chain = prompt | RunnableParallel(
            completion=self.llm, prompt_value=RunnablePassthrough()
        )
        ...
        if parser:
            ...
            def _do_parsing_retrying(x: dict):
                ...
                try:
                    result = retry_planner_parser.parse_with_prompt(
                        completion=completion, prompt_value=prompt_value
                    )
                except OutputParserException:
                    prompt_value = cast(PromptValue, x["prompt_value"])
                    logger.warning(
                        "Proceeding without result due to parser errors (even after retrying). "
    ...
    def _sample_data(self, n_samples: int):
        ...
        if self.data is not None:
            ...
            self.data = self.data.sample(
                n=n_samples, random_state=random.randint(0, 10000)
            ).reset_index(
                drop=True
...
class SummarisationChain(BaseChain):
    ...
    async def _generate_summaries(self, data: List[str]) -> List[str]:
        chain = self.create_chain(
            system_template=generate_summary_system_prompt(),
    ...
    async def _evaluate_summary_quality(
        self, summary: List[str], text: List[str]
    ) -> List[SummaryQualitySchema]:
        ...
        response = await chain.abatch(
            inputs=[{"text": i, "summary": j} for i, j in zip(text, summary)]
        )
        return response
    ...
    async def postprocess(self, summaries: List[str], data: List[str]) -> Dataset:
        ...
        logger.info(
            f"Summary Quality: {np.mean([int(sample.score) for sample in quality])}"
        )
    ...
    async def _generate_genetic_evolution(self, dataset: Dataset) -> str:
        ...
        genetic_evolver = GeneticEvolver(
            initial_population=dataset.data[dataset.data_col].tolist(),
            generations=10,
            mutation_rate=0.1,
        )
...
class AspectSummarisationChain(BaseChain):
    ...
    async def _generate_aspect_summaries(
        self, data: List[str], aspect: str
    ) -> List[str]:
        chain = self.create_chain(
            system_template=generate_aspect_summarisation_prompt(aspect=aspect),
            human_template=generate_summary_human_prompt(),
        )
        summaries = await chain.abatch(inputs=[{"text": i} for i in data])
        return summaries
    ...
    async def _evaluate_summary_quality(
        self, summary: List[str], text: List[str], aspect: str
    ) -> List[SummaryQualitySchema]:
        parser = PydanticOutputParser(pydantic_object=AspectSummarisationQualitySchema)
        chain = self.create_chain(
            system_template=generate_aspect_summarisation_evaluation_system_prompt(
                aspect=aspect
            ),
            human_template=check_summary_quality_human_prompt(),
            parser=parser,
    ...
    async def run(
        ...
        data = self._sample_data(
            dataset.data, n_examples
        )  # Use the new sampling method
        logger.info(f"Running aspect summarisation chain on {n_examples} examples")
        summaries = await self._generate_aspect_summaries(
            data[dataset.data_col].tolist(), aspect=aspect
        )
        summaries = [i["completion"].content for i in summaries]
        dataset.data["generated"] = summaries
        self.generated = dataset.data
        logger.info(f"Summary examples: {random.sample(summaries, 1)}")
        quality = await self._evaluate_summary_quality(
            summaries, data[dataset.data_col].tolist(), aspect=aspect
        )
        logger.info(
            f"Summary Quality: {np.mean([int(sample.score) for sample in quality])}"
        )
...
class RAGChain(BaseChain):
    ...
    async def run(
        self, dataset: Dataset, n_examples: int = 3, do_genetic_evolution: bool = False
    ) -> str:
        ...
        rag_examples = await self._generate_rag_examples(
            self.data[dataset.data_col].tolist()
        )
...
class QuizChain(BaseChain):
    ...
    async def _generate_quiz(self, dataset: Dataset) -> str:
        ...
        chain = self.create_chain(
            system_template=generate_quiz_system_prompt(),
            human_template=generate_quiz_human_prompt(),
            parser=parser,
        )
        quiz = await chain.abatch(
            inputs=[{"text": i} for i in dataset.data[dataset.data_col].tolist()]
        )
        return quiz
...
class FreeQueryChain(BaseChain):
    ...
    async def _generate_free_query_instruction(
        self, examples: List[str], solutions: List[str]
    ) -> List[FreeQueryScheme]:
        parser = PydanticOutputParser(pydantic_object=FreeQueryScheme)
        # TODO: add few-shot instruction prompt
        chain = self.create_chain(
            system_template=generate_instruction_one_shot_system_prompt(),
            human_template=generate_instruction_one_shot_human_prompt(),
            parser=parser,
        )
        free_query_instruction = await chain.abatch(
            inputs=[{"text": i, "result": j} for i, j in zip(examples, solutions)]
        )
        return free_query_instruction

    async def _combine_free_query_instruction(
        self, free_query_instruction: List[FreeQueryScheme]
    ) -> str:
        parser = PydanticOutputParser(pydantic_object=FreeQueryMerger)
        chain = self.create_chain(
            system_template=merge_instructions(),
            human_template=merge_instructions_human_prompt(),
            parser=parser,
        )

        merged_instruction = await chain.abatch(
            inputs=[
                {"text": [json.dumps(i.model_dump()) for i in free_query_instruction]}
            ]
        )
        return merged_instruction

    async def _generate_result_with_instruction(
        self, instruction: FreeQueryMerger, text: List[str], num_retries: int = 3
    ) -> str:
        ...
        chain = self.create_chain(
            system_template=complete_system_prompt,
    ...
    async def run(
        self, dataset: Dataset, n_examples: int = 3, initial_instruction: str = None
    ) -> str:
        self.data = copy.deepcopy(dataset.data)
        self._sample_data(n_samples=n_examples)
        logger.info(
            f"Running free query chain on {n_examples} examples: Instruction Extraction"
        )
        free_query_instruction = await self._generate_free_query_instruction(
            dataset.labeled_data[dataset.data_col].tolist(),
            dataset.labeled_data[dataset.labels_col].tolist(),
        )
        logger.info(
            f"Running free query chain on {n_examples} examples: Instruction Merging"
        )
        merged_instruction = await self._combine_free_query_instruction(
            free_query_instruction
        )
        logger.info(
            f"Running free query chain on {n_examples} examples: Result Generation"
        )
        result = await self._generate_result_with_instruction(
            merged_instruction[0], dataset.data[dataset.data_col].tolist()
        )
