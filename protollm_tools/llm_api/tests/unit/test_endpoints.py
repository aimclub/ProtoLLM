...
async def test_generate_endpoint(test_app, test_local_config):
    async with AsyncClient(
        ...
        with patch(
            "protollm_api.backend.endpoints.send_task", new_callable=AsyncMock
        ) as send_task_mock, patch(
            "protollm_api.backend.endpoints.get_result", new_callable=AsyncMock
        ) as get_result_mock:
            ...
            send_task_mock.assert_called_once_with(
                test_local_config, "llm_api-queue", transaction_model, ANY
            )

            get_result_mock.assert_called_once_with(
                test_local_config, "test-job-id", ANY
            )
...
async def test_chat_completion_endpoint(test_app, test_local_config):
    async with AsyncClient(
        ...
        with patch(
            ...
            response = await client.post("/chat_completion", json=prompt)

            prompt_data = ChatCompletionModel.model_validate_json(json.dumps(prompt))
            transaction_model = ChatCompletionTransactionModel(
                prompt=prompt_data, prompt_type=PromptTypes.CHAT_COMPLETION.value
            )
            send_task_mock.assert_called_once_with(
                test_local_config, "llm_api-queue", transaction_model, ANY
            )
