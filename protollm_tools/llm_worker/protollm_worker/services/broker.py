...
class LLMWrap:
    ...
    def __init__(self, llm_model: BaseLLM, config: Config):
        """
    ...
    def _callback(self, ch, method, properties, body):
        ...
        logger.info(
            f"The LLM response for task {transaction.prompt.job_id} has been generated"
        )
        logger.info(f"{self.redis_prefix}:{transaction.prompt.job_id}\n{func_result}")
        self.redis_bd.save_item(
            f"{self.redis_prefix}:{transaction.prompt.job_id}", {"content": func_result}
        )
        logger.info(
            f"The response for task {transaction.prompt.job_id} was written to Redis"
        )
