...
class OpenAPILLM(APIlLLM, BaseLLM):
    ...
    def __init__(
        ...
        self.client = OpenAI(api_key=token, base_url=model_url)
        self.handlers = {
    ...
    def __call__(
        self, transaction: PromptTransactionModel | ChatCompletionTransactionModel
    ):
        """
    ...
    def generate(
        ...
        **kwargs,
        ...
        logger.info(
            f"Starting generation for single prompt: {prompt.content} with temperature {temperature}"
        )
        try:
    ...
    def create_completion(
        ...
        **kwargs,
        ...
        messages = [
            {"role": unit.role, "content": unit.content} for unit in prompt.messages
        ]
        try:
            result = self._chat_completion(messages, temperature, tokens_limit, model)
            return result
        except Exception as ex:
            msg = f"The response generation has been interrupted. Error: {ex}."
            logger.error(msg)
