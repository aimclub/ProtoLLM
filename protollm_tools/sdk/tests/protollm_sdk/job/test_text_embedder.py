...
@pytest.mark.ci
@pytest.mark.parametrize("custom_timeout", [30, 60, 120])
def test_text_embedder_custom_timeout(custom_timeout):
    """
    Tests that the custom timeout value is correctly set.
    """
    text_embedder = TextEmbedder(
        text_emb_host="localhost", text_emb_port=8080, timeout_sec=custom_timeout
    )
...
def text_embedder():
    ...
    """
    return TextEmbedder(
        text_emb_host=Config.text_embedder_host, text_emb_port=Config.text_embedder_port
    )
...
def test_text_embedder_connection_error(text_embedder, text_embedder_request):
    ...
    with patch.object(text_embedder.client, "post") as mock_post:
        ...
        with pytest.raises(
            Exception,
            match="The embedding was interrupted. Error: The LLM server is not available.",
        ):
