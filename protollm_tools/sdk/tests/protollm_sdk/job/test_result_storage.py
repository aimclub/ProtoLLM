...
def test_for_job(result_storage):
    ...
    with patch("protollm_sdk.jobs.result_storage.ResultStorage") as mock_result_storage:
        ...
        mock_result_storage.assert_called_once_with(
            redis_host="redis://localhost:6379", redis_port=None, prefix="new_job"
        )
...
def test_save_dict_success(mock_redis, result_storage):
    ...
    mock_redis_instance.set.assert_called_once_with(
        "test_job:12345", json.dumps({"status": "success"})
    )
...
def test_save_dict_failure(mock_redis, result_storage):
    ...
    with pytest.raises(
        Exception,
        match="Saving the result with the key test_job:12345 has been interrupted",
    ):
        result_storage.save_dict("12345", {"status": "success"})
...
def test_load_dict_failure(mock_redis, result_storage):
    ...
    with pytest.raises(
        Exception,
        match="Failed to load the result with the key test_job:12345 from redis.",
    ):
