...
from protollm_sdk.jobs.job_invoke import (
    BlockingJobResult,
    WorkerJobResult,
    JobInvoker,
    InvokeType,
...
def blocking_job_result():
    ...
    mock_storage.for_job.return_value.load_dict.return_value = {
        "status": "success",
        "job_id": "12345",
    }

    job_result = BlockingJobResult(
        storage=mock_storage, job_name="test_job", job_id="12345"
    )
...
def test_blocking_job_result_timeout(mock_sleep, blocking_job_result):
    ...
    with patch(
        "time.monotonic", side_effect=[0, 1, 2, 300]
    ):  # Simulate waiting 0/1/2/300 seconds
...
def test_blocking_job_result_retries_and_succeeds(mock_sleep, blocking_job_result):
    ...
    mock_storage.for_job().load_dict.side_effect = [
        None,
        None,
        None,
        {"status": "success", "job_id": "12345"},
    ]
...
def test_worker_job_result_timeout(mock_sleep, worker_job_result):
    ...
    with patch(
        "time.monotonic", side_effect=[0, 1, 2, 300, 600]
    ):  # Simulate waiting 0/1/2/300 seconds
        with pytest.raises(
            TimeoutError, match="Couldn't retrieve result in 10min 0sec"
        ):
...
def test_job_invoker_worker_success(job_invoker):
    ...
    mock_task.apply_async.assert_called_once_with(
        args=(mock_job_class, mock.ANY), kwargs={}
    )
