...
def cleanup_queues(rabbit_wrapper):
    ...
    with rabbit_wrapper.get_channel() as channel:
        channel.queue_declare(
            queue="test_priority_queue", durable=True, arguments={"x-max-priority": 5}
        )
...
def test_publish_message_with_priority(rabbit_wrapper):
    ...
    with rabbit_wrapper.get_channel() as channel:
        ...
        ), "Message in the queue does not match the sent message"
        assert (
            header_frame.priority == priority
        ), f"Expected priority {priority}, but got {header_frame.priority}"
...
def test_consume_message(rabbit_wrapper):
    ...
    assert (
        consumed_messages[0] == message
    ), "Consumed message does not match the sent message"
