...
def cleanup_redis(redis_wrapper):
    with patch.object(
        redis_wrapper, "_get_redis", return_value=MagicMock()
    ) as mock_redis:
        mock_redis().flushall()
...
def test_save_item_publishes_message(redis_wrapper, mock_redis):
    ...
    mock_message = {"channel": key.encode("utf-8"), "data": b"set"}
    ...
    assert (
        message is not None
    ), "Message was not received from the channel within the timeout"
...
def test_get_item_raises_exception(redis_wrapper, mock_redis):
    ...
    mock_redis_instance = mock_redis.return_value
    mock_redis_instance.get.side_effect = Exception(
        f"The receipt of the element with the {key} prefix was interrupted"
    )
...
def test_save_item_raises_exception(redis_wrapper, mock_redis):
    ...
    mock_redis_instance = mock_redis.return_value
    mock_redis_instance.set.side_effect = Exception(
        f"Saving the result with the {key} prefix has been interrupted"
    )
...
def test_check_key_raises_exception(redis_wrapper, mock_redis):
    ...
    mock_redis_instance = mock_redis.return_value
    mock_redis_instance.get.side_effect = Exception(
        f"An error occurred while processing the {key} key"
    )
...
async def test_wait_item(redis_wrapper, mock_async_redis):
    ...
    mock_pubsub.get_message = AsyncMock(side_effect=[None, mock_message])
...
async def test_wait_item_raises_exception(redis_wrapper, mock_async_redis):
    ...
    mock_redis_instance.get = AsyncMock(
        side_effect=Exception(
            f"The receipt of the test element with the {key} prefix was interrupted"
        )
    )
    mock_redis_instance.pubsub.side_effect = lambda: mock_pubsub
    ...
    with pytest.raises(
        Exception,
        match=f"The receipt of the test element with the {key} prefix was interrupted",
    ):
